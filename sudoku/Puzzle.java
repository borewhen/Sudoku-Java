...
public class Puzzle {
      ...
      public void newPuzzle(int cellsToGuess) {
         ...
         int toAdd = (int)(Math.random() * 9);

         for (int row = 0; row < GameBoardPanel.GRID_SIZE; ++row) {
            for (int col = 0; col < GameBoardPanel.GRID_SIZE; ++col) {
               numbers[row][col] = ((hardcodedNumbers[row][col] + toAdd) % 9 + 1);
            }
         }
         ...
         for (int emptyCells = cellsToGuess; emptyCells > 0; emptyCells--) {
            isGiven[(int)(Math.random() * (9 - 1) + 1)][(int)(Math.random() * (9 - 1) + 1)]
               = false;
         }
